...
每一轮导演交互产生一组事件, 按顺序追加到事件日志中。所有系统状态 (全局状态、人物状态、记忆、场景、生效指令等) 都可以从事件序列重建。回滚 = 从事件序列尾部移除事件。这是事件溯源 (Event Sourcing) 架构。
...
移除当前轮次的所有事件 (director_input + narrative_output + 所有衍生事件), 回滚状态到上一轮结束时的状态。
...
由于记录了每一个事件, 重写的回退就是挨个移除事件, 直到回退到目标轮次之前。
