...
public sealed class StateRepository : IStateRepository
{
    ...
    public async Task<IReadOnlyList<StateChangeCapture>> CaptureStateChangesAsync
    (
        long              sessionID,
        long              roundID,
        CancellationToken cancellationToken = default
    )
    {
        ...
        var logs = await connection.QueryAsync
                   (
                       """
                       SELECT attribute_id, scene_id, old_value, new_value, source, reason
                       FROM state_change_logs
                       WHERE round_id = @roundID AND session_id = @sessionID
                       ORDER BY id ASC
                       """,
        ...
        var firstPerAttribute = new Dictionary<long, (string OldValue, string NewValue, long SceneID, string Source, string Reason)>();
        ...
        foreach (var log in logs)
        {
            ...
            {
                var existing = firstPerAttribute[attrID];
                firstPerAttribute[attrID] = (existing.OldValue, (string)log.new_value, existing.SceneID, existing.Source, existing.Reason);
            }
        }
    ...
    public async Task ReplayStateChangesAsync
    (
        long                              sessionID,
        long                              targetRoundID,
    ...
    {
        var originalOldValues = originalChanges.ToDictionary(c => c.AttributeID, c => c.OldValue);
        ...
        try
        {
            foreach (var change in tempChanges)
            {
                ...
                (
                    """
                    INSERT INTO state_values (attribute_id, session_id, value, updated_at)
                    VALUES (@attrID, @sessionID, @newValue, @updatedAt)
                    ON CONFLICT(attribute_id, session_id)
                    DO UPDATE SET value = @newValue, updated_at = @updatedAt
                    """,
                    new
                    {
                ...
                (
                    ...
                    INSERT INTO state_change_logs
                        (attribute_id, session_id, scene_id, round_id, old_value, new_value, source, reason, created_at)
                    VALUES (@attrID, @sessionID, @sceneID, @roundID, @oldValue, @newValue, @source, @reason, @createdAt)
                    """,
                    new
                    {
                        attrID = change.AttributeID,
